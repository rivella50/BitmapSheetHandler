...
Utility for Actionscript 3 and "TexturePacker":http://www.texturepacker.com/ users to handle their image sheets within a project.
...
# Create your (different DPI) sheets and configs in TexturePacker and publish them into your project's asset folder
# In your project create a separate Object config for every sheet/json combination:
...
bq. or with multiple sheets to import into the same handler:
...
bq.	 
# If you only have one sheet to handle use DPI_SINGLE, else one of the existing DPI_160, DPI_240 or DPI_320 constants
# You get notified when the last combination has finished importing, where you can then ask the handler for Bitmap or MultiDPIBitmapSource copies:
...
bq. or with multiple sheets you get back a MultiDPIBitmapSource:
